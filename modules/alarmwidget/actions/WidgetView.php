...
class WidgetView extends CControllerDashboardWidgetView {
	...
	protected function doAction(): void {
		...
		// Se "Only" estiver marcado, precisamos buscar os suprimidos na engine
		...
		$data = CScreenProblem::getData([
			...
			'tags' => $tags,
			'show_symptoms' => false,
			'show_suppressed' => $engine_show_suppressed, // Passamos a lógica OR
			'acknowledgement_status' => $ack_status,
			'show_opdata' => 0
		], $search_limit);

		// 4. Hidratação (APENAS Host e OpData)
		// Não buscamos status de Ack/Sup aqui para evitar conflito
		$triggerIds = [];
		...
		if (!empty($data['problems'])) {
			foreach ($data['problems'] as $problem) {
				// Recupera status REAIS do problema (vindos da Engine)
				$p_sup = (int)($problem['suppressed'] ?? 0);
				
				// --- FILTRO CRÍTICO: ONLY SUPPRESSED ---
				// Se a opção "Only Suppressed" está ligada (1)
				// E o problema atual NÃO é suprimido (0)
				// Então PULAMOS este problema.
				...
				$problems_final[] = [
					...
					'ack_count' => $p_ack,
					'suppressed' => $p_sup, // Passa o valor correto para a View
