...
class CWidgetNavTreeNext extends CWidget {
	...
	onFeedback({type, value}) {
		if (type === CWidgetsData.DATA_TYPE_MAP_ID) {
			...
			for (const [id, item] of Object.entries(this.#navtree)) {
				if (item.link_type == 0 && item.sysmapid == sysmapid && item.parent == this.#navtree_item_selected) {
					new_item_id = id;
					break;
				}
			}
			...
			if (new_item_id == 0) {
				for (const [id, item] of Object.entries(this.#navtree)) {
					if (item.link_type == 0 && item.sysmapid == sysmapid && item_selected.parent == id) {
						new_item_id = id;
						break;
					}
				}
		...
		else if (type === CWidgetsData.DATA_TYPE_DASHBOARD_ID) {
			const dashboardid = value[0];
			const item_selected = this.#navtree[this.#navtree_item_selected];
			let new_item_id = 0;

			for (const [id, item] of Object.entries(this.#navtree)) {
				if (item.link_type == 1 && item.dashboardid == dashboardid && item.parent == this.#navtree_item_selected) {
					new_item_id = id;
					break;
				}
			}
			...
			if (new_item_id == 0) {
				for (const [id, item] of Object.entries(this.#navtree)) {
					if (item.link_type == 1 && item.dashboardid == dashboardid && item_selected.parent == id) {
						new_item_id = id;
						break;
					}
				}
	...
	processUpdateResponse(response) {
		...
		if (response.navtree_data !== undefined) {
			...
			this.#dashboards_accessible = response.navtree_data.dashboards_accessible;
	...
	#buildTree(parent_id = 0) {
		...
		for (const i in this.#navtree) {
			const tree_item = {
				...
				link_type: this.#navtree[i].link_type || 0,
				sysmapid: this.#navtree[i].sysmapid || 0,
				dashboardid: this.#navtree[i].dashboardid || 0,
				url: this.#navtree[i].url || ''
			};
			...
			if (tree_item.parent == parent_id) {
				...
				// Check accessibility based on link type
				if (tree_item.link_type == 0) {
					tree_item.item_active = (tree_item.sysmapid == 0 || this.#maps_accessible.includes(tree_item.sysmapid));
				}
				else if (tree_item.link_type == 1) {
					tree_item.item_active = (tree_item.dashboardid == 0 || this.#dashboards_accessible.includes(tree_item.dashboardid));
				}
				else {
					tree_item.item_active = true; // URLs are always active
				}
	...
	#makeTreeItem(item, depth = 1, editable = true) {
		...
		const has_link = (item.link_type == 0 && item.sysmapid != 0) 
			|| (item.link_type == 1 && item.dashboardid != 0) 
			|| (item.link_type == 2 && item.url != '');

		if (this.isEditMode() && !has_link) {
		...
		if (!this.isEditMode() && !item.item_active) {
			li_item.classList.add('inaccessible');
		}

		let link;

		if (!this.isEditMode() && item.item_active && has_link) {
			link = document.createElement('a');
			link.href = '#';
			
			if (item.link_type == 0) {
				link.setAttribute('data-sysmapid', item.sysmapid);
				link.setAttribute('data-link-type', '0');
			}
			else if (item.link_type == 1) {
				link.setAttribute('data-dashboardid', item.dashboardid);
				link.setAttribute('data-link-type', '1');
			}
			else if (item.link_type == 2) {
				link.setAttribute('data-url', item.url);
				link.setAttribute('data-link-type', '2');
			}
		}
		else {
		...
		if (item.link_type == 0 && item.sysmapid != 0) {
			li_item.setAttribute('data-sysmapid', item.sysmapid);
		}
		else if (item.link_type == 1 && item.dashboardid != 0) {
			li_item.setAttribute('data-dashboardid', item.dashboardid);
		}
		...
		if (editable) {
			...
			arrow_btn.addEventListener('click', () => {
				...
				if (this.getWidgetId() !== null) {
					updateUserProfile(`web.dashboard.widget.navtreenext.item-${branch.getAttribute('data-id')}.toggle`,
		...
		if (this.isEditMode() && editable) {
			...
			const link_type_fld = document.createElement('input');
			link_type_fld.id = `${this.getUniqueId()}_navtree.${item.id}.link_type`;
			link_type_fld.type = 'hidden';
			link_type_fld.name = `navtree[${item.id}][link_type]`;
			link_type_fld.value = item.link_type || 0;
			li_item.appendChild(link_type_fld);
			...
			mapid_fld.value = item.sysmapid || 0;
			li_item.appendChild(mapid_fld);

			const dashboardid_fld = document.createElement('input');
			dashboardid_fld.id = `${this.getUniqueId()}_navtree.${item.id}.dashboardid`;
			dashboardid_fld.type = 'hidden';
			dashboardid_fld.name = `navtree[${item.id}][dashboardid]`;
			dashboardid_fld.value = item.dashboardid || 0;
			li_item.appendChild(dashboardid_fld);

			const url_fld = document.createElement('input');
			url_fld.id = `${this.getUniqueId()}_navtree.${item.id}.url`;
			url_fld.type = 'hidden';
			url_fld.name = `navtree[${item.id}][url]`;
			url_fld.value = item.url || '';
			li_item.appendChild(url_fld);
		}
	...
	#getDefaultSelectable() {
		return jQuery('.tree-item:visible', jQuery(this._target))
			.not('[data-sysmapid="0"][data-dashboardid="0"]')
			.first()
			.data('id');
	}

	#broadcast() {
		const item = this.#navtree[this.#navtree_item_selected];
		
		if (item.link_type == 0 && item.sysmapid) {
			this.broadcast({
				[CWidgetsData.DATA_TYPE_MAP_ID]: [item.sysmapid]
			});
		}
		else if (item.link_type == 1 && item.dashboardid) {
			this.broadcast({
				[CWidgetsData.DATA_TYPE_DASHBOARD_ID]: [item.dashboardid]
			});
		}
	}

	#updateUserProfile() {
		updateUserProfile('web.dashboard.widget.navtreenext.item.selected',
	...
	#updateWidgetFields() {
		...
		jQuery('input[name$="[name]"]', jQuery(this._body)).each((index, field) => {
			...
			const link_type = document.getElementById(`${prefix}navtree.${id}.link_type`).value;
			const sysmapid = document.getElementById(`${prefix}navtree.${id}.sysmapid`).value;
			const dashboardid = document.getElementById(`${prefix}navtree.${id}.dashboardid`).value;
			const url = document.getElementById(`${prefix}navtree.${id}.url`).value;
			...
			if (link_type != 0) {
				this._fields.navtree[id].link_type = link_type;
			}
			...
			if (dashboardid != 0) {
				this._fields.navtree[id].dashboardid = dashboardid;
			}

			if (url != '') {
				this._fields.navtree[id].url = url;
			}
		});
	}
	...
	#itemEditDialog(id, parent, depth, trigger_element) {
		...
		url.setArgument('action', 'widget.navtreenext.item.edit');
		...
		jQuery.ajax({
			...
			data: {
				...
				link_type: item_edit ? this._target.querySelector(`[name="navtree[${id}][link_type]"]`).value : 0,
				sysmapid: item_edit ? this._target.querySelector(`[name="navtree[${id}][sysmapid]"]`).value : 0,
				dashboardid: item_edit ? this._target.querySelector(`[name="navtree[${id}][dashboardid]"]`).value : 0,
				url: item_edit ? this._target.querySelector(`[name="navtree[${id}][url]"]`).value : '',
				depth: depth
			},
			dataType: 'json',
			success: (response) => {
					...
					'buttons': [
						{
							...
							'action': (overlay) => {
								...
								url.setArgument('action', 'widget.navtreenext.item.update');
								...
								overlay.xhr = $.ajax({
									...
									data: {
										...
										link_type: form_inputs.link_type.value,
										sysmapid: form_inputs.sysmapid !== undefined
											? form_inputs.sysmapid.value
											: '0',
										dashboardid: form_inputs.dashboardid !== undefined
											? form_inputs.dashboardid.value
											: '0',
										url: form_inputs.url !== undefined
											? form_inputs.url.value
											: '',
									...
									success: (response) => {
										...
										if ('error' in response) {
											...
											if (item_edit) {
												...
												jQuery(`[name="navtree[${id}][link_type]"]`, $row).val(response['link_type']);
												jQuery(`[name="navtree[${id}][sysmapid]"]`, $row).val(response['sysmapid']);
												jQuery(`[name="navtree[${id}][dashboardid]"]`, $row).val(response['dashboardid']);
												jQuery(`[name="navtree[${id}][url]"]`, $row).val(response['url']);
												jQuery('> .tree-row > .content > .item-name', $row)
													.empty()
													.attr('title', response['name'])
													.append(jQuery('<span>').text(response.name));
												
												const has_link = (response.link_type == 0 && response.sysmapid != 0)
													|| (response.link_type == 1 && response.dashboardid != 0)
													|| (response.link_type == 2 && response.url != '');
												$row.toggleClass('no-map', !has_link);
											}
											else {
	...
	#activateContentEvents() {
		if (this.#has_content && this.getState() === WIDGET_STATE_ACTIVE) {
			if (this.isEditMode()) {
				...
				for (const link of this._target.querySelectorAll('a[data-sysmapid], a[data-dashboardid], a[data-url]')) {
