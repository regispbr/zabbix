...
namespace Widgets\GeomapPlus\Actions;
...
class WidgetView extends CControllerDashboardWidgetView {
	...
	protected function doAction(): void {
		...
		if ($this->getInput('initial_load', 0)) {
			...
			$data['config'] = $this->geomap_config + $this->getMapCenter() + [
				...
				'marker_shape' => $this->fields_values['marker_shape'],
				'colors' => [
					'no_problems' => '#' . $this->fields_values['color_no_problems'],
					'information' => '#' . $this->fields_values['color_information'],
					'warning' => '#' . $this->fields_values['color_warning'],
					'average' => '#' . $this->fields_values['color_average'],
					'high' => '#' . $this->fields_values['color_high'],
					'disaster' => '#' . $this->fields_values['color_disaster']
				]
			];
		}
	...
	private function transformGroupName(string $name): string {
		...
		if (@preg_match($pattern, '') === false) {
			// Invalid regex, return original name
			return $name;
		}

		$transformed = @preg_replace($pattern, $replacement, $name);
		
		// If regex didn't match or error occurred, return original name
	...
	private function getMapCenter(): array {
		...
		$user_default_view = CProfile::get('web.dashboard.widget.geomapplus.default_view', '', $this->widgetid);
	...
	private function getUserProfileFilter(): array {
		return [
			'severity' => CProfile::get('web.dashboard.widget.geomapplus.severity_filter', [], $this->widgetid)
		];
	}
