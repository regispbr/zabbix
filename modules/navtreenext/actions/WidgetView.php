...
class WidgetView extends CControllerDashboardWidgetView {
	...
	protected function doAction(): void {
		// Get list of sysmapids and dashboardids.
		$sysmapids = [];
		$dashboardids = [];
		$navtree_items = [];

		foreach ($this->fields_values['navtree'] as $id => $navtree_item) {
			$link_type = array_key_exists('link_type', $navtree_item) ? $navtree_item['link_type'] : 0;
			$sysmapid = 0;
			$dashboardid = 0;
			$url = '';

			if ($link_type == 0 && array_key_exists('sysmapid', $navtree_item)) {
				$sysmapid = $navtree_item['sysmapid'];
				if ($sysmapid != 0) {
					$sysmapids[$sysmapid] = true;
				}
			}
			elseif ($link_type == 1 && array_key_exists('dashboardid', $navtree_item)) {
				$dashboardid = $navtree_item['dashboardid'];
				if ($dashboardid != 0) {
					$dashboardids[$dashboardid] = true;
				}
			}
			elseif ($link_type == 2 && array_key_exists('url', $navtree_item)) {
				$url = $navtree_item['url'];
			}
		...
		$dashboards_accessible = $dashboardids
			? API::Dashboard()->get([
				'output' => [],
		...
		if ($widgetid) {
			$pattern = 'web.dashboard.widget.navtreenext.item-%.toggle';
			...
			$navtree_item_selected = CProfile::get('web.dashboard.widget.navtreenext.item.selected', 0, $widgetid);
		}
		...
		$this->setResponse(new CControllerResponseData([
			...
			'dashboards_accessible' => array_keys($dashboards_accessible),
