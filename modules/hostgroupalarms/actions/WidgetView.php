    ...
    protected function doAction(): void {
        ...
        if (!empty($data['problems'])) {
            foreach ($data['problems'] as $problem) {
                ...
                // Pega o primeiro host
                $host = reset($trigger['hosts']);
                if (!$host) continue;

                // Validação de segurança para chaves do host
                $host_id = $host['hostid'] ?? 0;
                $host_maintenance_status = $host['maintenance_status'] ?? 0;
                ...
                // --- BLINDAGEM CONTRA NULOS (Resolve o erro do log) ---
                ...
                $is_suppressed = defined('ZBX_PROBLEM_SUPPRESSED') 
                    ? ($p_sup == ZBX_PROBLEM_SUPPRESSED) 
                    : ($p_sup == 1);

                $is_acknowledged = defined('EVENT_ACKNOWLEDGED')
                    ? ($p_ack == EVENT_ACKNOWLEDGED)
                    : ($p_ack == 1);
                ...
                $detailed_alarms[] = [
                    ...
                    'host_name' => $host_name, // Agora garantido que é string
                    'clock' => $p_clock,
                    'acknowledged' => $is_acknowledged ? 1 : 0,
