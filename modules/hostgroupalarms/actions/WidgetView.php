	...
	protected function doAction(): void {
		...
		if (!empty($data['problems'])) {
			foreach ($data['problems'] as $problem) {
				...
				// --- CORREÇÃO DE CHAVES E CONSTANTES ---
				// Garantimos que as chaves existam com valor default
                $p_name = $problem['name'] ?? 'Unknown problem';
                $p_ack = (int)($problem['acknowledged'] ?? 0);
                $p_sup = (int)($problem['suppressed'] ?? 0);
                
                // ZBX_PROBLEM_SUPPRESSED é constante global, mas se falhar, usamos 1
